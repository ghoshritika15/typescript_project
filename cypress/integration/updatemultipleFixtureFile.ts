            ...
            describe(fixtureData.context, () => {
                ...
                before(function () {
                    cy.fixture(fixtureData.name).then(function (testData) {
                        ...
                        cy.readFile(`cypress/fixtures/${fixtureData.name}.json`).then((data) => {
                            data.testName = 'After Update'
                            // cy.writeFile(fixtureData.name + ".json", data)
                            cy.writeFile('writeData.json', data)
                            cy.log(data)
                        })
                    })
                })
